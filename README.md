...
### A. New server — app + database together

```bash
docker compose up -d --build
docker compose logs -f app        # the admin password is printed here, once
```

That is the whole setup. No `.env`, no editing. The signing secret and the
admin password are generated on first boot — the secret is kept in the database
so sessions survive restarts, and the password is printed once:

```
================================================================
  ADMIN ACCOUNT CREATED — this password is shown ONCE
    email:    admin@example.com
    password: eRf9mHHnQyTqJ9pj
================================================================
```

Open `http://<server-ip>:8080`, sign in, change the password under Settings.

Optional overrides (only if you want them) go in `.env`: `ADMIN_EMAIL`,
`ADMIN_PASSWORD`, `JWT_SECRET`, `APP_PORT`, `DB_PASSWORD`.

### B. You already run PostgreSQL in Docker
...
`setup-db.sh` finds your PostgreSQL container and its network, creates a
separate `monit` role and database inside it, writes `.env`, verifies the
connection, and runs the migrations. Your existing databases are never touched
and the container is never restarted. Re-running it is safe.

Non-interactive:

```bash
./setup-db.sh -c my-postgres -u postgres -p 8080 -y
```

### Behind nginx

To serve under a sub-path, build for it and proxy with matching trailing
slashes — see [docs/NGINX.md](docs/NGINX.md):

```bash
echo 'VITE_BASE=/monit/' >> .env
docker compose -f docker-compose.app-only.yml build --no-cache app
```

### Add a server

From the central server, in one command — it registers the host, fetches the
key, installs, and verifies:

```bash
MONIT_ADMIN_PASSWORD='…' ./deploy-agent.sh root@10.1.0.101 -a -e you@company.com
```

Or run the installer on the target and answer the prompts:

```bash
sudo ./install.sh
```

Change settings later with `sudo /opt/monit/monit-config.sh` — it validates,
sends a test sample, rolls back if the server rejects it, and restarts for you.
