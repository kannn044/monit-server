...
export default async function ingestRoutes(app) {
  app.post('/api/v1/ingest', {
    ...
    // Only advance last_seen for current samples (not old buffered backlog)
    await q(
      `UPDATE servers SET
         last_seen = GREATEST(COALESCE(last_seen, 'epoch'), $2::timestamptz),
         os = COALESCE(NULLIF($3, ''), os)
       WHERE id = $1`,
      [s.server_id, ts.toISOString(), s.os || '']
    );
